...
class MediaAssetsTableView extends GetView<MediaAssetsController> {
  ...
  static const String _tableKey = 'media_assets';
  static const String _tableLabel = 'Media Assets';
  static final List<TableColumnDefinition> _columnDefinitions = [
    TableColumnDefinition(field: 'id', label: 'Id', isAudit: false),
    TableColumnDefinition(field: 'image', label: 'Image', isAudit: false),
    TableColumnDefinition(field: 'title', label: 'Title', isAudit: false),
    TableColumnDefinition(field: 'properties', label: 'Properties', isAudit: false)
  ];
  ...
  List<_ColumnSpec<MediaAssetsModel>> _buildAllColumnSpecs() {
    return [
    _ColumnSpec<MediaAssetsModel>(
      field: 'id',
      label: 'Id',
      isAudit: false,
    ...
    _ColumnSpec<MediaAssetsModel>(
      ...
      label: 'Image',
      isAudit: false,
      cellBuilder: (context, m) => DataCell(Text(m.image == null ? '' : m.image.toString())),
    ),
    _ColumnSpec<MediaAssetsModel>(
      field: 'title',
      label: 'Title',
    ...
    _ColumnSpec<MediaAssetsModel>(
      ...
      label: 'Properties',
      isAudit: false,
      cellBuilder: (context, m) => DataCell(Text(m.properties?.id?.toString() ?? '')),
    )
    ];
  }

  _ColumnSpec<MediaAssetsModel>? _findSpec(String field, List<_ColumnSpec<MediaAssetsModel>> specs) {
    ...
    return AppShell(
      ...
      body: Obx(() {
        ...
        var specs = visibleDefs
            ...
            .whereType<_ColumnSpec<MediaAssetsModel>>()
            .toList();
        if (specs.isEmpty) {
          specs = List<_ColumnSpec<MediaAssetsModel>>.from(allSpecs);
        }
...
// Pagination bar
  ...
  Widget _buildRowActions(BuildContext context, MediaAssetsModel m) {
    return Row(
  ...
  Widget _buildSortLabel(BuildContext context, MediaAssetsController controller, String field, String label, String? activeField, bool isDescending) {
  ...
  void _toggleSort(MediaAssetsController controller, String field) {
  ...
  Future<bool?> _openFormDialog(BuildContext context, {required String title}) {
    return _showFormDialog(context, title: title, body: MediaAssetsForm());
  }
