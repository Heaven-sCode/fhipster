// generators/role_middleware_generator.js
// Emits lib/core/auth/role_middleware.dart
// - GetX middleware to enforce required authorities/roles on a route
// - If user is not authenticated -> /unauthorized
// - If authenticated but lacks role(s) -> /forbidden
// - Supports ANY (default) or ALL semantics
//
// Usage in routes (from routes_generator):
//   middlewares: [RoleMiddleware(requiredAuthorities: const ['ROLE_ADMIN'])]
//
// Or require ALL:
//   middlewares: [RoleMiddleware(requiredAuthorities: const ['ROLE_ADMIN','ROLE_MANAGER'], requireAll: true)]

function generateRoleMiddlewareTemplate() {
...
module.exports = { generateRoleMiddlewareTemplate };
